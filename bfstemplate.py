...
def levelOrderBottom(self, root: Optional[TreeNode]) -> List[List[int]]:
	...
	while queue:
		...
		for i in range(levelSize):
			...
			if curNode.right:
				queue.append(curNode.right)
		res.appendleft(curLevel)   # <------ This is the only place changed compared to Leetcode 102
	return res
...
def zigzagLevelOrder(self, root: Optional[TreeNode]) -> List[List[int]]:
	...
	while queue:
		...
		for i in range(levelSize):
			curNode = queue.popleft()
			if leftToRight:
				curLevel.append(curNode.val)   # <---- depending on the falg, we pick append() or appendleft()
			else:
...
def averageOfLevels(self, root: Optional[TreeNode]) -> List[float]:
	...
	while queue:
		...
		res.append(curSum/levelSize)   # <----- push the average to the res list
	return res
